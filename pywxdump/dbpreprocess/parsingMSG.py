...
class ParsingMSG(DatabaseBase):
    ...
    def msg_count(self, wxid: str = "", last_week=False):
        """
        ...
        if wxid:
            if last_week:
                sql = f"SELECT StrTalker, COUNT(*), CreateTime FROM MSG WHERE StrTalker='{wxid}' AND datetime(CreateTime, 'unixepoch') >= date('now', '-7 days');"
            else:
                sql = f"SELECT StrTalker, COUNT(*) FROM MSG WHERE StrTalker='{wxid}';"
        else:
            if last_week:
                sql = f"SELECT StrTalker, COUNT(*), CreateTime FROM MSG WHERE datetime(CreateTime, 'unixepoch') >= date('now', '-7 days') GROUP BY StrTalker ORDER BY COUNT(*) DESC;"
            else:
        ...
        if not result:
            return {}
        if last_week:
            df = pd.DataFrame(result, columns=["wxid", "msg_count", "create_time"])
        else:
    ...
    def msg_list(self, wxid="", start_index=0, page_size=500, last_week=False):
        if wxid:
            if last_week:
                sql = (
                    "SELECT localId, IsSender, StrContent, StrTalker, Sequence, Type, SubType,CreateTime,MsgSvrID,DisplayContent,CompressContent,BytesExtra,ROW_NUMBER() OVER (ORDER BY CreateTime ASC) AS id "
                    "FROM MSG WHERE StrTalker=? AND datetime(CreateTime, 'unixepoch') >= date('now', '-7 days')"
            ...
            if last_week:
                sql = (
                    "SELECT localId, IsSender, StrContent, StrTalker, Sequence, Type, SubType,CreateTime,MsgSvrID,DisplayContent,CompressContent,BytesExtra,ROW_NUMBER() OVER (ORDER BY CreateTime ASC) AS id "
                    "FROM MSG WHERE datetime(CreateTime, 'unixepoch') >= date('now', '-7 days') ORDER BY CreateTime ASC LIMIT ?,?")
            else:
                sql = (
                    "SELECT localId, IsSender, StrContent, StrTalker, Sequence, Type, SubType,CreateTime,MsgSvrID,DisplayContent,CompressContent,BytesExtra,ROW_NUMBER() OVER (ORDER BY CreateTime ASC) AS id "
